...
// In Production, we do NOT fall back to defaults effectively.
// We might define them here to avoid crashes, but login logic will reject them.
const ENV_JWT_SECRET = process.env.JWT_SECRET;
const ENV_PASSWORD = process.env.DASHBOARD_PASSWORD;
...
export async function isAuthenticated() {
    try {
        ...
        // In Production, REJECT if using default secret
        const effectiveSecret = ENV_JWT_SECRET || 'default-dev-secret-change-in-production-xyz123';

        if (IS_PROD && !ENV_JWT_SECRET) {
            logger.error('Login disabled: JWT_SECRET not set in production');
            return false;
        }
...
export function verifyPassword(password) {
    ...
    // In Production: Strict checks
    if (IS_PROD) {
        if (!ENV_PASSWORD) {
            logger.error('Login disabled: DASHBOARD_PASSWORD not set');
            return false;
        }
        if (ENV_PASSWORD === 'admin123') {
            logger.error('Login disabled: Default password used in production');
            return false;
        }
    }

    // Determine the active password
    // In Dev: Fallback to 'admin123' if not set
    const activePassword = ENV_PASSWORD || 'admin123';
...
export function createAuthToken() {
    ...
    if (IS_PROD && !ENV_JWT_SECRET) {
        throw new Error('Cannot issue tokens: JWT_SECRET not set');
    }
